...
class SampleGateway
  ...
  def mo_received(transceiver, pdu)
   ...
   if !(body =~ /[a-zA-Z0-9]{6}/)
		self.send_mt("SoLienLacDT",pdu.source_addr,"sai cu phap")
		return
	end
	p = PhatTem.where("PIN=?",pdu.short_message.downcase).first
	if(p.nil?)
		self.send_mt("SoLienLacDT",pdu.source_addr,"ma chua kick hoat")
		return
	end

	#8137 8537
	if(p.tem_type == '5' && pdu.destination_addr == '8137')
		self.send_mt("SoLienLacDT",pdu.source_addr,"sai tong dai, de nghi gui den 8537")
		return
	end

	if !(p.kh.nil?)
		self.send_mt("SoLienLacDT",pdu.source_addr,"san pham da duoc kick hoat vao ngay  #{p.updated_at}")
		p.so_kh = p.so_kh.next
		return
	end
	
	self.send_mt("SoLienLacDT",pdu.source_addr,"#{p.san_pham.sms_trave} thank you")
	p.kh = "1"
	p.updated_at = Time.now
	p.so_kh = p.so_kh.next
	p.sdt = pdu.source_addr
	p.save	
  end
